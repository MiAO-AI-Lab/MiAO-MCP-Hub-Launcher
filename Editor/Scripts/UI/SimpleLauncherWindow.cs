...
namespace com.MiAO.MCP.Launcher.UI
{
    ...
    /// Independent launcher without external dependencies
    /// </summary>
    public class SimpleLauncherWindow : EditorWindow
    {
        private const string MENU_TITLE = "MCP Hub Launcher";
        
        private const int MIN_WIDTH = 900;
        private const int MIN_HEIGHT = 600;
        
        private const string STYLE_PATH = "SimpleLauncherWindow";
        ...
        private static SimpleLauncherWindow s_Instance;
        ...
        // Properties
        public static SimpleLauncherWindow Instance => s_Instance;
        ...
        {
            s_Instance = GetWindow<SimpleLauncherWindow>(false, MENU_TITLE, true);
        ...
        private void LoadStyles()
        {
            ...
            {
                Debug.LogWarning($"[MCP Hub] Could not load MCP Hub styles: {ex.Message}");
            }
        }
        ...
        private void ConfigureActionButton(Button button, ExtensionPackageInfo extension)
        {
            ...
            if (extension.Id == HUB_CORE_FRAMEWORK_ID)
            {
                if (extension.IsInstalled)
                {
                    ...
                    {
                        button.text = "Update";
                        button.clickable = new Clickable(() => UpdateExtension(extension));
                    }
                    else
                    {
        ...
        private void LoadData()
        {
            ...
            Debug.Log($"[MCP Hub] Loaded {m_AvailableExtensions.Count} extensions");
        }
        ...
        private void RefreshExtensionList()
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[MCP Hub] Error refreshing extension list: {ex}");
            }
        }
        ...
        private async void InstallExtension(ExtensionPackageInfo extension)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[MCP Hub] Installation failed: {ex}");
            }
            finally
            {
        ...
        private async void UninstallExtension(ExtensionPackageInfo extension)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[MCP Hub] Uninstallation failed: {ex}");
            }
            finally
            {
                SetButtonLoadingState(button, false, originalText);
        ...
        private async void UpdateExtension(ExtensionPackageInfo extension)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[MCP Hub] Update failed: {ex}");
            }
            finally
            {
                SetButtonLoadingState(button, false, originalText);
